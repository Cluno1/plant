 ...
 * @LastEditTime: 2025-06-01 01:17:51
 ...
 * @returns
 */
export function judgePeriod(nowData, type, plantKind, plantPeriod) {
    //"/plants/1/1.jpg",
    let result = {
    ...
    switch (type) {
        case "sun":
            if (nowData[type] >= plantContent?.lowestSun &&
            ...
            break;
        case "tempr":
            if (nowData[type] >= plantContent.lowestTempr &&
        ...
        case "water":
            if (nowData[type] > plantContent.water) {
                ...
                //刚好合适的浇水量
                if (nowData.sun >= plantContent?.lowestSun &&
                    nowData.sun <= plantContent?.heightestSun)
                    if (nowData.tempr >= plantContent.lowestTempr &&
 ...
 * @returns
 */
export function getPlantDetail(index) {
    switch (index) {
        case 1:
            return strawberry;
        case 2:
